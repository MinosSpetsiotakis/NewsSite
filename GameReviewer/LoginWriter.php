...
if(isset($_POST['login']))   // it checks whether the user clicked login button or not 
{
    ...
    $user = $_GET['user'];
    $pass = $_GET['pass'];
    $sql = "SELECT auid FROM authors WHERE username = '".$user."'AND password = '".$pass."'" ;
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            echo '<script type="text/javascript"> window.open("Author.php?user='.$row["auid"].'","_self");</script>';   
        }
    } else {
         ...
         <?php
            ...
            if (isset($_POST['login']) && !empty($_POST['username']) && !empty($_POST['password'])) {
                ...
                if ($conn->connect_error) {
                 ...
                 $sql = "SELECT auid FROM authors WHERE username = '".$_POST['username']."'AND password = '".$_POST['password']."'" ;
                $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                 while($row = $result->fetch_assoc()) {
                    $_SESSION['user']=$row["auid"];
                     $_SESSION['valid'] = true;
